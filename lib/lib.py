...
def press_play_playlist(driver:ChromiumDriver):
    driver.find_element(By.XPATH, '//*[@id="main"]/div/div[2]/div[4]/div[1]/div[2]/div[2]/div/main/section/div[2]/div[2]/div[2]/div/div/div[1]/button').click()

def press_play_song(driver:ChromiumDriver):
...
def enable_repeat(driver: ChromiumDriver):
    ...
    repeat_song_state = driver.find_element(By.XPATH, '//*[@id="main"]/div/div[2]/div[3]/footer/div/div[2]/div/div[1]/div[2]/button[2]').get_attribute("aria-label")
    print(repeat_song_state)

def open_uri(driver: ChromiumDriver, spotify_song_uri: str):
    driver.get(spotify_song_uri)
    keyboard.press_and_release('esc')

def try_accept_cookies(driver, username):
    try:
        driver.find_element(By.XPATH, "//button[text()='Accept Cookies']").click()
        print(f"{username} Accepted Cookies")
    except Exception as e:
        print(f"{username} failed to accept accept cookies")
